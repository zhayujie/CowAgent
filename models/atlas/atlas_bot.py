...
"""
Atlas Cloud Bot — fully OpenAI-compatible.

Atlas Cloud (https://www.atlascloud.ai) is a full-modal AI inference platform that
exposes a single OpenAI-compatible endpoint for many model families (DeepSeek, Qwen,
GLM, Kimi, MiniMax, and more). This bot talks to ``/chat/completions`` exactly like the
OpenAI / DeepSeek bots, so it reuses the shared agent/tool/streaming/vision plumbing.

Config (config.json / available_setting):
- ``atlas_api_key``   : Atlas Cloud API key
- ``atlas_api_base``  : defaults to https://api.atlascloud.ai/v1
- ``model``           : e.g. deepseek-ai/deepseek-v4-pro (keep the vendor prefix)

Thinking mode notes (for DeepSeek-V4 models served by Atlas):
- Toggle: ``{"thinking": {"type": "enabled" | "disabled"}}`` (default: enabled)
- Effort: ``reasoning_effort`` ∈ {"high", "max"}
- Under thinking mode ``temperature``/``top_p``/penalties are ignored by the server; we
  drop them locally. ``reasoning_content`` is returned alongside ``content`` and, once a
  tool call appears in the history, MUST be echoed back on later turns.
- Give reasoning models enough ``max_tokens`` (>= 512), or the budget is spent on the
  thinking trace and ``content`` comes back empty with ``finish_reason=length``.
"""
...
DEFAULT_MODEL = const.ATLAS_DEEPSEEK_V4_PRO  # deepseek-ai/deepseek-v4-pro


class AtlasBot(Bot, OpenAICompatibleBot):
    def __init__(self):
        super().__init__()
    ...
    @property
    def api_key(self):
        return conf().get("atlas_api_key") or conf().get("open_ai_api_key")

    @property
    def api_base(self):
        url = (
            conf().get("atlas_api_base")
    ...
    def _model_supports_thinking(model_name: str) -> bool:
        """DeepSeek-V4 models on Atlas expose the explicit `thinking` switch."""
        if not model_name:
            return False
        m = model_name.lower()
        return "deepseek-v4" in m
    ...
    def reply(self, query, context=None):
        if context.type == ContextType.TEXT:
            logger.info("[ATLAS] query={}".format(query))
            ...
            logger.debug("[ATLAS] session query={}".format(session.messages))

            new_args = self.args.copy()
            reply_content = self.reply_text(session, args=new_args)
            logger.debug(
                "[ATLAS] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format(
            ...
            if reply_content["completion_tokens"] == 0 and len(reply_content["content"]) > 0:
                ...
                reply = Reply(ReplyType.ERROR, reply_content["content"])
                logger.debug("[ATLAS] reply {} used 0 tokens.".format(reply_content))
            return reply
        else:
    ...
    def reply_text(self, session, args=None, retry_count: int = 0) -> dict:
        try:
            ...
            if res.status_code == 200:
                ...
                logger.error(
                    f"[ATLAS] chat failed, status_code={res.status_code}, "
    ...
    def call_with_tools(self, messages, tools=None, stream: bool = False, **kwargs):
        """
        Call the Atlas Cloud API with tool support for agent integration.
        ...
        - Thinking mode toggle and reasoning_effort for DeepSeek-V4 models
        """
        try:
            ...
            # Thinking mode (DeepSeek-V4 only). Honour the toggle propagated by agent_bridge.
            ...
            logger.error(f"[ATLAS] call_with_tools error: {e}")
            import traceback
    ...
    def _handle_stream_response(self, request_body: dict):
        """Stream SSE chunks from Atlas and yield OpenAI-format deltas (with reasoning_content)."""
        try:
            headers = self._build_headers()
            ...
            if response.status_code != 200:
                ...
                logger.error(f"[ATLAS] API error: status={response.status_code}, msg={error_msg}")
            ...
            for line in response.iter_lines():
                ...
                try:
                    ...
                    logger.warning(f"[ATLAS] JSON decode error: {e}, data: {data_str[:200]}")
                    continue

                if chunk.get("error"):
                    error_data = chunk["error"]
                    error_msg = error_data.get("message", "Unknown error") if isinstance(error_data, dict) else str(error_data)
                    logger.error(f"[ATLAS] stream error: {error_msg}")
            ...
            logger.error("[ATLAS] Request timeout")
            yield {"error": True, "message": "Request timeout", "status_code": 500}
        except Exception as e:
            logger.error(f"[ATLAS] stream response error: {e}")
            import traceback
            logger.error(traceback.format_exc())
            yield {"error": True, "message": str(e), "status_code": 500}
    ...
    def _handle_sync_response(self, request_body: dict):
        ...
        try:
            ...
            # and round-trip it on tool-call turns (required by DeepSeek thinking models).
            ...
            logger.error(f"[ATLAS] sync response error: {e}")
            import traceback
            logger.error(traceback.format_exc())
            yield {"error": True, "message": str(e), "status_code": 500}
    ...
    def _convert_messages_to_openai_format(self, messages):
        ...
        Once any assistant turn in the history triggered a tool call, DeepSeek
        thinking models require `reasoning_content` on every subsequent assistant
        message until the next user turn; missing it returns 400. We back-fill an
        empty string when the trace was not captured (non-thinking backends ignore
        the field).
        """
        if not messages:
            return []
    ...
    def call_vision(self, image_url: str, question: str,
        ...
        """Analyse an image via the Atlas OpenAI-compatible /chat/completions endpoint."""
        try:
            vision_model = model or self.args.get("model", DEFAULT_MODEL)
            payload = {
                "model": vision_model,
            ...
            logger.error(f"[ATLAS] call_vision error: {e}")
