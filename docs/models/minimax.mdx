...
MiniMax-M3 natively accepts text, image, and video input through the OpenAI-compatible chat endpoint. Once `minimax_api_key` is configured, the Agent's Vision tool automatically routes image requests to the selected MiniMax-M3 model, with no need to specify a separate vision model.
