...
The Skills page in the **web console** and **desktop app** can add, edit, disable, and remove MCP servers without editing `mcp.json` by hand.

- Use **Test connection** to handshake with a server and list its tools before saving.
- Saving writes the Agent's `mcp.json` in the standard `mcpServers` format.
- CowAgent hot-reloads the file; the new tools are available on the **next message**. No process restart is required.
