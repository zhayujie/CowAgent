...
The `write` and `edit` tools cannot modify this file. Edit `mcp.json` yourself; a rewritten config would be hot-reloaded into a local subprocess.

### Stdio command allowlist

Stdio MCP servers spawn a local subprocess. CowAgent only launches executables listed in `config.json`:

```json
"mcp_stdio_command_allowlist": ["npx", "node", "python", "python3", "uvx"]
```

`config-template.json` includes this list. If the key is missing or empty, **no stdio server is started** (fail-closed). Remote SSE / Streamable HTTP servers are unaffected.
...
Edit `~/cow/mcp.json` directly (or the host `./cow/mcp.json` in Docker). After you save, send another message so changed servers are picked up. The Agent's `write` / `edit` tools refuse this path on purpose.
...
- **Hot reload**: When you modify `mcp.json`, changed servers are automatically reloaded after the current message — no need to restart cow. Newly added stdio servers whose `command` is not in `mcp_stdio_command_allowlist` are not started.
...
| Stdio MCP servers never start | Set `mcp_stdio_command_allowlist` in `config.json` (e.g. `["npx", "node", "python", "python3", "uvx"]`). An empty or missing list denies all stdio commands |
| A specific server fails to load | Look for `[MCP] Server 'xxx' load failed` in startup logs — usually missing dependencies, API keys, or a command not on the allowlist |
...
| Docker deployment | Make sure host's `./cow` is mounted to `/home/agent/cow` in the container, then drop `mcp.json` into host's `./cow/` |
