...
| MiniMax | MiniMax-M3 | Uses the selected native multimodal model |

<Note>
  ZhipuAI and MiniMax text-only models do not support image understanding; MiniMax-M3 is selected automatically for native multimodal input.
</Note>
