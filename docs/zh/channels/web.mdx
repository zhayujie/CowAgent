...
- **工作空间 / 模型 / 权限**：在输入框下方可为当前会话单独设置项目工作空间、模型和权限；使用多个项目时，历史会话会自动按项目分组。详见 [项目架构 - 项目工作空间](/zh/intro/architecture#project-workspace)
...
可为当前会话设置权限模式，控制 Agent 对文件与命令的操作范围：只读（read-only）、工作空间可写（workspace-write）、完全访问（full-access）。工具调用因权限被拦截时会给出可点击的提示，便于就地调整。

<Frame caption="会话级权限模式：只读、工作空间可写、完全访问">
  <img src="https://cdn.jsdelivr.net/gh/zhayujie/cowagent-assets@main/screenshots/zh/web-console-access-mode.png" style={{ maxWidth: "600px" }} />
</Frame>
