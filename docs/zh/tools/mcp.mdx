...
Web 控制台和桌面客户端的「技能」页面可以直接添加、编辑、停用和删除 MCP 服务器，不必再手工改 `mcp.json`。

- 用 **测试连接** 先做握手并列出工具，确认无误再保存。
- 保存会写入该 Agent 的 `mcp.json`，格式与 Claude / Cursor 的 `mcpServers` 一致。
- CowAgent 会热加载该文件，**下一条消息**即可使用新工具，无需重启进程。
