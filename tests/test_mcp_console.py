...
def _get(handler_cls, params):
    ...
    with patch.object(skills_api, "_require_auth"), \
         patch.object(skills_api.web, "header"), \
         patch.object(skills_api.web, "input", return_value=skills_api.web.storage(**params)):
...
def _send(handler_cls, method, body):
    ...
    with patch.object(skills_api, "_require_auth"), \
         patch.object(skills_api.web, "header"), \
         patch.object(skills_api.web, "data", return_value=json.dumps(body).encode()):
...
def test_skills_handler_install_and_delete(tmp_path):
    ...
    dummy = DummyService()
    with patch("channel.web.api.skills._skill_service", return_value=dummy), \
         patch("channel.web.api.skills._install_skill_for_agent", return_value=fake):
...
def test_skills_handler_refuses_builtin_delete():
    ...
    with patch("channel.web.api.skills._skill_service", return_value=DummyService()):
