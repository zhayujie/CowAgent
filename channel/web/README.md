...
### Comparing against the pre-split version

The pre-split frontend (`chat.html` + `console.js` + `console.css`) is not kept
as a copy; git history is the only source. To compare, check out a snapshot
first:

```
python channel/web/tools/snapshot_legacy.py          # defaults to master
python channel/web/tools/snapshot_legacy.py <ref>    # or any ref
```

The output goes to `static/legacy/` (gitignored; delete it when done). Then:
...
instance from a `git worktree` with `COW_WEB_PORT`, but that brings up a second
full backend: the scheduler and IM channels would connect twice, and a
configured Feishu app would receive every message twice. Do not do this unless
you only run the web channel.
