...
class ChatChannel(Channel):
        ...
        if reply and reply.type:
            ...
            if not e_context.is_pass() and reply and reply.type:
                if reply.type in self.NOT_SUPPORT_REPLYTYPE:
                    ...
                    reply.content = _t(
                        "不支持发送的消息类型: ", "Unsupported message type: "
                    ) + str(reply.type)
        ...
        if reply and reply.type:
            ...
            if not e_context.is_pass() and reply and reply.type:
                ...
                # the text (feishu) or natively renders STT under the voice (wechatcom).
                    ...
                    and context.get("voice_reply_text")
    ...
    def produce(self, context: Context):
        ...
        agent_bridge = None
        # 角色指令快速路径必须在 route_context 之前执行：若会话已绑定到
        # 已停用/被删除的 agent，route_context 会抛 AgentUnavailableError，
        # 用户将无法再通过 /角色 /回到大海 自救。
        if context.type == ContextType.TEXT and context.content:
            try:
                ...
                logger.warning(f"[chat_channel] Failed to get agent bridge: {e}")
                agent_bridge = None
            if agent_bridge is not None:
                from channel.role_switch import handle_role_command

                handled, reply_text = handle_role_command(
                    context.content,
                    self.channel_type,
                    session_id,
                    agent_bridge,
                    bot_name=getattr(self, "name", None) or "",
                )
                if handled and reply_text:
                    self._send_reply(context, Reply(ReplyType.TEXT, reply_text))
                    return
        ...
        try:
            ...
            messages = {
                ...
                SteerStatus.CLOSING: _t(
                    "当前任务已结束，无法再引导。",
                    "The active task is already finishing.",
                ),
